...
to-report select [ columns table ]
...
to-report take [ n lst ]
  report sublist lst 0 n
end

to-report tail [ n lst ]
  report sublist lst (length lst - n) length lst
end

to-report drop-meta [ table ]
  let columns first modes map length table
  report filter [ length ? = columns ] table
end
